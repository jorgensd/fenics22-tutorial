...
# + [markdown] slideshow={"slide_type": "slide"} tags=[]
# ## Crouzeix-Raviart {cite}`crouzeix-raviart`

# + [markdown] slideshow={"slide_type": "notes"} tags=[]
# Alternatively, the same order convergence can be achieved using fewer degrees of freedom if a Crouzeix-Raviart element is used for the velocity space.
# -
...
# + [markdown] slideshow={"slide_type": "slide"} tags=[]
# ## Piecewise linear pressure space {cite}`crouzeix-falk`

# + [markdown] slideshow={"slide_type": "notes"} tags=[]
# When using a piecewise linear pressure space, we could again try using a velocity space one degree higher, but we would again observe that there is no convergence. In order to achieve convergence, we can augment the quadratic space with a cubic bubble function on the triangle.
# -
